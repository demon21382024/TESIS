...
\textit{Person Re-ID} tiene como objetivo determinar si imágenes o secuencias procedentes de distintas cámaras pertenecen a la misma persona. En el contexto del reconocimiento basado en la marcha (\textit{Gait Recognition}), esta tarea se aborda desde el análisis temporal. La complejidad radica en la variabilidad intra-clase debido a los cambios de pose, velocidad de caminata, oclusiones o resolución; así como en la reducida variabilidad inter-clase entre individuos con patrones de movimiento similares. Esta dualidad exige representaciones altamente discriminativas y robustas.
...
Aunque este trabajo se centra en \textit{Gait Recognition}, la literatura de \textit{Person Re-ID} basada en apariencia y en pre-entrenamiento autosupervisado aporta fundamentos metodológicos, como el aprendizaje contrastivo, el modelado de regiones ocultas (\textit{Masked Image Modeling, MIM}) y estrategias de aumento de datos. No obstante, dichas técnicas fueron diseñadas principalmente para imágenes estáticas. Por tal motivo, su adaptación a secuencias de marcha requiere considerar explícitamente la coherencia temporal, la dinámica del movimiento y la estructura del cuerpo humano en el tiempo. Esta revisión crítica incorpora y analiza métodos representativos de \textit{Gait Recognition}, con el objetivo de identificar estrategias de pre-entrenamiento autosupervisado que puedan extraerse al modelado espacio-temporal.

La presente revisión de literatura busca sintetizar y comparar como los distintos métodos autosupervisados diseñan señales de entrenamiento para capturar características discriminativas en datos secuenciales. Además, se busca analizar la evidencia experimental obtenida, configuraciones espacio-temporales y métricas de desempeño. Finalmente, se discuten las limitaciones comunes en la literatura, como el alto costo computacional de los modelos temporales y la necesidad de grandes volúmenes de datos no etiquetados y la limitada capacidad para generalizar ante variaciones significativas a la vista.
...
A partir del análisis del estado del arte, se identificaron diversas limitaciones y oportunidades de investigación relevantes bajo el contexto del reconocimiento de personas basado en la marcha. En primer lugar, una parte de los métodos de aprendizaje autosupervisado ha sido desarrollada principalmente sobre datos de apariencia estática; por ejemplo, en enfoques como LUPerson \cite{Zheng2022GaitLU1M}, PASS \cite{Zheng2022PASS} o PersonViT \cite{Huang2025PersonViT}, lo que limita su uso directo en escenarios donde la información temporal es fundamental. En este sentido, existe una oportunidad clara en el diseño de tareas de pre-entrenamiento que incorporen explícitamente la dinámica de la marcha y la coherencia espacio-temporal de las secuencias. \\
Por otro lado, si bien los enfoques basados en representaciones estructurales, como aquellos que emplean esqueletos y modelos tipo GNN; por ejemplo, SM-SGE \cite{Han2021SMSGE} o CAGES \cite{Han2021LocalitySGE}, han mostrado resultados prometedores, su desempeño se encuentra condicionado por la disponibilidad y calidad de datasets, tales como KS20 o KGBD. Alternativamente, la estimación de esqueletos a partir de datos RGB; como es el caso en CASIA-B, introduce una fuente adicional de ruido que puede afectar negativamente la calidad de las representaciones aprendidas.

Asimismo, los modelos recientes basados en arquitecturas tipo \textit{Transformer} presentan una alta demanda computacional, lo que puede limitar su aplicación en escenarios con recursos restringidos. En contraste, métodos específicos como GaitGL \cite{Lin2022GaitGL} o GaitTAKE \cite{Hsu2022GaitTAKE}, han demostrado la importancia de capturar de manera eficiente patrones de la marcha y poder modelar adecuadamente la información temporal. \\
En este contexto, la literatura sugiere que los enfoques híbridos que combinan pre-entrenamiento autosupervisado con ajuste fino supervisado construyen una base de investigación prometedora. Particularmente, el diseño de tareas pretexto alineadas con la naturaleza secuencial del problema y con las métricas finales de evaluación podría favorecer la obtención de representaciones más robustas y generalizables. De esta manera, la integración de aprendizaje autosupervisado con mecanismos de modelado temporal surgen como una alternativa para abordar efecientemente las limitaciones, permitiendo aprovechar datos no etiquetados, reducir la dependencia de anotaciones y mejorar el rendimiento en escenarios realistas.