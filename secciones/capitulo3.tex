...
En este sentido, las estrategias de aprendizaje auto-supervisado han demostrado ser una alternativa eficaz para aprovechar la abundancia de secuencias sin etiquetas, permitiendo capturar la dinámica de la marcha mediante tareas. Sin embargo, su efectividad depende en gran medida de la capacidad del modelo para representar adecuadamente la estructura temporal de las secuencias, lo cual refuerza la necesidad de enfoques que modelen explícitamente la dinámica delmovimiento humano. 
...
\section{Representación Espacio-Temporal}
...
\section{invariancia y normalización física}
...
Una estrategia más eficiente, consiste en eliminar dichas variaciones mediante normalización física antes del entrenamiento. Bajo este contexto, el alineamiento basado en el centro de masa (CoM) permite centrar las siluetas en cada \textit{frame}, reduciendo el efecto de la traslación y estabilizando la secuencia. Esto permitirá que el modelo se centre en la dinámica real del movimiento.

\section{Aprendizaje métrico temporal}

El objetivo del reconocimiento de marcha es aprender representaciones que permitan distinguir entre individuos en un espacio de características. Para ello, el aprendizaje métrico optimiza directamente la distancia entre muestran en función de su identidad. Por ejemplo, el \textit{triplet-loss}, busca minimizar la distancia entre muestras de la misma identidad y maximizarla respecto a diferentes identidades.

En el caso de secuencias de marcha, este enfoque se extiende al dominio temporal, donde los \textit{embeddings} deben capturar la dinámica del movimiento, dando lugar a representaciones más robustas y discrimnativas.
...
El enfoque propuesto se sustenta en la necesidad de modelar de manera explícita la dinámica temporal de la marcha, considerando que la identidad de un individuo está estrechamente ligada a patrones de movimientos que evolucionanen el tiempo. \\
Asimismo, la incorporación de técnicas de normalización física permite reducir variaciones irrelevantes en las secuencias, facilitando que el modelo se enfoque en características discriminativas. En conjunto, el uso de representaciones espacio-temporales, mecanismos de alineamiento y estrategias de aprendizaje métrico, contribuyen a la construcción de \textit{embeddings} más robustos. \\
En este sentido, el uso de un esquema híbrido de aprendizaje, que combine aprendizaje autosupervisado y supervisado, se integra como un componente adicional que permite mejorar la generalización y precisión del modelo.