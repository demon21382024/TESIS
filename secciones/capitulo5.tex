...
El modelo propuesto en la presenta investigación está diseñado para operar sobre secuencias temporales de marcha, en lugar de imágenes individuales. A diferencia de enfoques tradicionales basados en \textit{frames} independientes, esta propuesta modela explícitamente la dinámica del movimiento humano, permitiendo capturar patrones temporales asociados al ciclo de marcha. 

La arquitectura combina un extractor espacial basado en redes convolucionales con un mecanismo de agregación temproal, generando representaciones secuenciales robustas que posteriormente son optimizadas mediante aprendizaje métrico. 

\section{Diseño híbrido y dinámica temporal}
...
La estrategia híbrida adoptada en este trabajo se plantea como un componente dentro del proceso de aprendizaje. Su propósito será aprovechar las ventajas del aprendizaje autosupervisado para mejorar la generalización, y del aprendizaje supervisado para refinar la discriminación entre identidades.

No obstante, el eje central de la metodología no radica únicamente en la combinación de paradigmas, sino en la capacidad del modelo para capturar y representar la dinámica temporal de la marcha, aspecto fundamental en tareas basadas en secuencias.
...
El pre-entrenamiento autosupervisado implementa en esta investigación se basa en la consistencia temporal entre subsecuencias de una misma secuencia de marcha. Donde, en lugar de aplicar transformaciones geométricas estáticas, se generan pares positivos a partir de segmentos temporales parcialmente solapados extraídos de una misma secuencia. Por ejemplo, una secuencia de longitud mayor se divide en dos subsecuencias que representan distintas fases del ciclo de marcha.

El modelo es entrenado para maximizar la similitud entre estas subsecuencias en el espacio de características, promoviendo la invariancia frente a la fase temporal del movimiento. Este enfoque permite capturar de manera más efectiva la dinámica temporal de la marcha, generando representaciones más robustas y coherentes con la naturaleza secuencial del problema. 
...
Las estrategias de augmentación se aplican a nivel de secuencia, incluyendo variaciones como recortes temporales, inversión horizontal y perturbaciones leves en los frames, con el objetivo de reforzar la robustez del modelo frente a variaciones espaciales y temporales. Todas estas operaciones están alineadas con el enfoque contrastivo implementado en el proyecto y buscan reforzar la robustez del modelo ante variaciones espaciales comunes en secuencias de marcha. Estas augmentaciones se aplican exclusivamente durante el pre-entrenamiento autosupervisado, mientras que la fase supervisada utiliza únicamente las transformaciones estándar.
...
\subsection{Construcción de secuencias temporales}

A diferencia de enfoques basados en imágenes individuales, el modelo propuesto requiere como entrada secuencias de longitud fija. Para ello, se construyen subsecuencias de tamaño $T$ a partir de las secuencias originales del dataset. 

Se aplica un muestreot emporal uniforme para seleccionar $T$ \textit{frames} representativos del ciclo de marcha. En casod e secuencias más largas, se generan múltiples subsecuencias.

\subsection{Alineamiento por Centro de Masa (CoM)}

Antes de la construcción de las secuencias, se aplica un proceso de alineamiento espacial basado en el centro de masa de la silueta. Este procedimiento consiste en trasladar el baricentro de los píxeles activos al centro geométrico de la imagen. El objetivo de este paso es eliminar variaciones de traslación lateral, permitiendo que el modelo concentre su capacidad en el aprendizaje de patrones estructurales y dinámicos, en lugar de aprender invariacias irrelevantes.
